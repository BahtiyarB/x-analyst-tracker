...
>
> **Tarih:** 2026-07-08 · **Durum:** ✅ TAMAM (v5 — thread çekme; v4 — HTML çıktı formatı).
> **v5 (2026-07-08):** Verilen/bookmark edilen tweet thread ise tüm self-thread çekilir.
> Ad-hoc: `fetch_thread.py`. Bookmark: `build_threads.py` + `build_html.py` "## Thread" bölümü.
> Detay: aşağıda **Adım 9**.
...
### Adım 9 — Thread çekme (v5, 2026-07-08)
**İstek:** Verilen ya da bookmark edilen bir tweet **thread** ise, tek tweet'le yetinme,
**tüm self-thread'i** çek.

**Çekirdek mekanizma:** TweetDetail GraphQL yaniti `threaded_conversation_with_injections_v2`
tüm konusmayi verir. `thread_util.reconstruct_thread(raw, focal_id)` fokal tweet'ten baslayip
**ayni yazarin `in_reply_to` zincirini** takip eder (fokal → self-reply → self-reply …) → sirali
thread. t.co genisletme + HTML-unescape + uzun-form `note_tweet` tam metni.

**İki giris noktasi:**
- **Ad-hoc** (`fetch_thread.py`): `uv run --no-project python fetch_thread.py <url|id> --out ~/Downloads --json`
  → temiz `.txt` (+ `.thread.json`). Thread degilse tek tweet'i verir. Bir X linki verilince bunu kullan.
- **Bookmark entegrasyonu** (Faz 2): `build_threads.py` mevcut TweetDetail raw'larindan
  (`x_articles_raw/` bedava + `x_threads_rawjson/` toplu-cekilen) thread olanlari `x_threads_raw/<id>.json`'a
  yazar; `build_html.py` bunu okuyup HTML'e **"## Thread (devamı)"** bölümü ekler (fokal HARIÇ, devam tweet'leri).

**Faz 2 toplu cekim (~517 non-article bookmark):**
```bash
# _filemap'te olup x_articles_raw'da olmayan id'ler:
python3 -c "import csv,os;R=os.path.dirname('.');ids=[r['tweet_id'] for r in csv.DictReader(open('_filemap.csv'))];[print(i) for i in ids if not os.path.exists(f'x_articles_raw/{i}.json')]" > /tmp/thread_ids.txt
# rate-limited batch (fetch_articles.mjs deseni; 429'da 15dk bekleyip rerun — idempotent):
node fetch_articles.mjs --file /tmp/thread_ids.txt x_threads_rawjson
...
| `x_tweetdetail.mjs` | **(v5)** Ortak TweetDetail GraphQL istemcisi (cookie + fetch). CLI: `node x_tweetdetail.mjs <id>` | tweet id | ham TweetDetail JSON |
| `thread_util.py` | **(v5)** TweetDetail'den yazarin self-thread reply-zincirini kurar + txt/html formatlar | ham JSON | thread dict / txt / html |
| `fetch_thread.py` | **(v5)** Ad-hoc: URL/ID → tüm self-thread'i indir | X status URL/id | `<slug>.txt` (+`.thread.json`) |
| `build_threads.py` | **(v5)** Bookmark raw'larindan thread'leri ayikla → `x_threads_raw/` | `_filemap.csv` + raw JSON'lar | `x_threads_raw/<id>.json`, `x_threads_checked.json` |
