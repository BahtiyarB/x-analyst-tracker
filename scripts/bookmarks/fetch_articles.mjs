// Bird cookies + TweetDetail GraphQL with withArticlePlainText=true.
...
const cookieRes = await extractCookiesFromChrome('Profile 2');
if (!cookieRes.cookies.authToken || !cookieRes.cookies.ct0) {
  console.error('Cookie extraction failed:', cookieRes.error);
  process.exit(1);
}
const { authToken, ct0 } = cookieRes.cookies;
...
if (arg[0] === '--id') {
  const r = await fetchTweetDetail(arg[1]);
  ...
  for (const id of ids) {
    ...
    const r = await fetchTweetDetail(id);
    if (r.ok) {
